...
pub fn execute(input: &str) -> String {
    ...
    (0..blocks.len()).par_bridge().map(|i| {
        let mut moved:HashSet<usize> = Default::default();
        let mut q:VecDeque<usize> = Default::default();
        q.push_back(i);
        while let Some(cur) = q.pop_front() {
            moved.insert(cur);
            supports[cur].iter().for_each(|x| {
                if supported_bys[*x].len() > 0 && supported_bys[*x].iter().all(|z| moved.contains(z)) {
                    q.push_back(*x)
                }
            });
        }
